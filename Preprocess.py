...
snow_augmenter = iaa.Sequential([
    iaa.imgcorruptlike.Snow(severity=4)  # severity can be adjusted from 1 to 5
])
...
num_to_keep = {'train': 1000, 'val': 200, 'test': 200}
        ...
        if os.path.exists(annotation_file):
            ...
            for line in lines:
                ...
                if len(parts) >= 6 and parts[5] == '4':
                    bbox_left = float(parts[0]) * scale_x/640
                    bbox_top = float(parts[1]) * scale_y/640
                    bbox_width = float(parts[2]) * scale_x/640
                    bbox_height = float(parts[3]) * scale_y/640
